...
class MMG:
    ...
    def output(self, rm, hmm_model, label_encoder):
        for _ in range(rm):
            map_length = 200
...
print (final_parsed_map)
# Get BaseModel object
bm_obj = base.BaseModel()

# Get Label Encoder
items = set(final_parsed_map)
label_encoder = bm_obj.get_label_encoder(items)
map_features = mario_obj.transform(label_encoder, final_parsed_map)
# print map_features
# Get HMM Model
...
# Write output map
mario_obj.output(rm, hmm_model, label_encoder)
